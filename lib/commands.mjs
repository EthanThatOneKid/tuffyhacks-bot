...
import { CHECKIN_ROLE_ID, BACKDOOR_DISCORD_ADMINS } from "./constants.mjs";
  ...
  [
    ...
    async (message) => {
      if (checkIsDM(message)) {
        return message.author.send("Try again in the check-in text channel.");
      } else {
        message.delete();
      }
      ...
      const err = await new Promise((res) =>
        checkins.findOne({ tag: message.author.tag }, (err, doc) =>
          err !== null
            ...
            : checkins.findOne({ email }, (err, doc) =>
                err !== null
                  ? res(err)
                  : doc === null
                  ? res({
      ...
      if (err === null) {
        if (message.guild !== null) {
          // TODO: Add "Camper" role upon checking in.
          // Read: https://github.com/AnIdiotsGuide/discordjs-bot-guide/blob/master/understanding/roles.md#add-a-member-to-a-role
          message.author.send(
